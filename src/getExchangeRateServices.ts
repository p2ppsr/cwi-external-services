...
export async function updateFiatExchangeRates(rates?: FiatExchangeRatesApi, updateMsecs?: number): Promise<FiatExchangeRatesApi> {
    ...
    if (!iorates.rates["USD"] || !iorates.rates[iorates.base])
        throw new ERR_BAD_REQUEST(`getExchangeRatesIo missing rates for 'USD' or base`)

    const r: FiatExchangeRatesApi = {
...
export async function getExchangeRatesIo(): Promise<ExchangeRatesIoApi> {
    ...
    if (r.status !== 200 || !r.data) {
        throw new ERR_BAD_REQUEST(`getExchangeRatesIo returned status ${r.status}`)
    }
